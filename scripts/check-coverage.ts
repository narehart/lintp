...
async function runTypeScriptCoverage(
  ...
  try {
    ...
    if (showTable) {
      ...
      for (const file of fileCoverages) {
        ...
        const line = `  ${formattedPath} L: ${lineColor}${formatPercentage(
          file.lines.pct
        )}${colors.reset} | B: ${branchColor}${formatPercentage(
          file.branches.pct
        )}${colors.reset} | F: ${funcColor}${formatPercentage(
          file.functions.pct
        )}${colors.reset} | S: ${stmtColor}${formatPercentage(
          file.statements.pct
        )}${colors.reset}`;
        console.log(line);
      }
    ...
    if (pct.statements.pct < thresholds.statements) {
      console.error(
        colorize(
          `\n✗ Statement coverage (${pct.statements.pct.toFixed(
            2
          )}%) is below threshold (${thresholds.statements}%)`,
          "red"
        )
      );
      failed = true;
    ...
    if (pct.branches.pct < thresholds.branches) {
      console.error(
        colorize(
          `✗ Branch coverage (${pct.branches.pct.toFixed(
            2
          )}%) is below threshold (${thresholds.branches}%)`,
          "red"
        )
      );
      failed = true;
    ...
    if (pct.functions.pct < thresholds.functions) {
      console.error(
        colorize(
          `✗ Function coverage (${pct.functions.pct.toFixed(
            2
          )}%) is below threshold (${thresholds.functions}%)`,
          "red"
        )
      );
      failed = true;
    ...
    if (pct.lines.pct < thresholds.lines) {
      console.error(
        colorize(
          `✗ Line coverage (${pct.lines.pct.toFixed(2)}%) is below threshold (${
            thresholds.lines
          }%)`,
          "red"
        )
...
async function runRustCoverage(verbose: boolean = false): Promise<boolean> {
  try {
    ...
    try {
      const jsonOutput: TarpaulinJson = JSON.parse(
      ...
      result = {
        ...
        branches: {
          ...
          percentage:
            branches.total > 0 ? (branches.covered / branches.total) * 100 : 0,
        },
        functions: {
      ...
      // Sort files by path
      const sortedFiles = Array.from(fileStats.entries()).sort((a, b) =>
    ...
    if (result.lines.percentage < threshold) {
      console.error(
        colorize(
          `\n✗ Rust line coverage (${result.lines.percentage.toFixed(
            2
