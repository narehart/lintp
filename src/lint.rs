...
pub fn run_lint(dir: &Path, config: &ParsedConfig, verbose: bool) -> Result<Vec<LintResult>> {
  ...
  let ignore_patterns: Vec<Pattern> = config.raw.lintp.ignore
    .iter()
  ...
  for entry in WalkDir::new(dir)
    ...
    let result = apply_rules(&name, path, is_dir, &applicable_rules, &config.parsed_matchers)?;
...
fn find_applicable_rules(
  ...
  for (glob_pattern, pattern_rules) in &config.path_rules {
    let pattern = Pattern::new(glob_pattern).with_context(||
...
fn apply_rules(
  ...
  if let Some(rule_str) = rules.get(&rule_key) {
    // Parse the rule
    let rule_expr = parse_expression(rule_str).with_context(||
      format!("Failed to parse rule: {}", rule_str)
    )?;
