...
pub fn parse_expression(input: &str) -> Result<Expression> {
  ...
  let pairs = DslParser::parse(Rule::top_level, input).map_err(|e|
    anyhow::anyhow!("Failed to parse expression: {}", e)
  )?;
  ...
  match pair.as_rule() {
    ...
    Rule::comparison_expr => {
      ...
      if let Some(op_pair) = inner.next() {
        let op = match op_pair.as_str() {
          ...
          _ => {
            return Err(anyhow::anyhow!("Unknown comparison operator: {}", op_pair.as_str()));
          }
        };
    ...
    Rule::string_literal => {
      ...
      if is_double_quoted && content.contains("${") {
          ...
          if let Some(start) = content[current_pos..].find("${") {
            ...
            if start > 0 {
              ...
              if !literal_text.is_empty() {
                template_parts.push(StringTemplatePart::Literal(literal_text.to_string()));
              }
            }
            ...
            if brace_count == 0 {
              ...
              let inner_pairs = DslParser::parse(Rule::expression, expr_content).map_err(|e| {
              ...
              template_parts.push(StringTemplatePart::Expression(Box::new(inner_expr)));

              current_pos = end_pos;
            } else {
              // Unmatched braces, treat as literal
              template_parts.push(StringTemplatePart::Literal(content[current_pos..].to_string()));
              break;
            }
          } else {
        ...
        if template_parts.iter().any(|p| matches!(p, StringTemplatePart::Expression(_))) {
    ...
    Rule::boolean_literal => {
      let value = match pair.as_str() {
        ...
        "false" => false,
        _ => {
          return Err(anyhow::anyhow!("Invalid boolean literal: {}", pair.as_str()));
        }
      };
    ...
    Rule::string_template => {
      ...
      if template_str.len() < 4 || !template_str.starts_with("${") || !template_str.ends_with("}") {
      ...
      let inner_pairs = DslParser::parse(Rule::expression, inner_expr_str).map_err(|e|
        anyhow::anyhow!("Failed to parse template expression: {}", e)
      )?;
...
mod tests {
  ...
  fn test_function_calls() -> Result<()> {
    ...
    let expr = parse_expression("any(map(siblings(\"*.js\"), without($item, \".js\")), $NAME)")?;
