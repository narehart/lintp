...
fn report_results(results: &[lint::LintResult]) -> bool {
  ...
  for result in results {
    match result {
      ...
      lint::LintResult::Failure { path, rule, message } => {
        success = false;
  ...
  if success {
    println!("{}", "All files and directories match the configured rules.".green());
  } else {
    println!("{}", "Some files or directories do not match the configured rules.".red());
  }
