...
fn test_find_function() -> Result<()> {
    ...
    let utils_js = call_function(
        "find",
        &[
            Value::String(utils_path.to_string_lossy().to_string()),
...
fn test_map_function_basic() -> Result<()> {
    ...
    let result = eval_list(
        "map(['hello.js', 'world.ts'], 'without($item, \".js\")')",
        &context,
    )?;
...
fn test_any_function() -> Result<()> {
    ...
    assert!(eval_bool(
        "any(['a.js', 'b.ts', 'c.jsx'], 'matches($item, /\\.js$/)')",
        &context
    )?);

    // Test any() with simple list - should not find match
    assert!(!eval_bool(
        "any(['a.ts', 'b.tsx', 'c.jsx'], 'matches($item, /\\.py$/)')",
        &context
    )?);
...
fn test_all_function() -> Result<()> {
    ...
    assert!(eval_bool(
        "all(['a.js', 'b.js', 'c.js'], 'matches($item, /\\.js$/)')",
        &context
    )?);

    // Test all() where not all items match
    assert!(!eval_bool(
        "all(['a.js', 'b.ts', 'c.js'], 'matches($item, /\\.js$/)')",
        &context
    )?);
...
fn test_filter_function() -> Result<()> {
    ...
    let long_names = eval_list(
        "filter(['ab.js', 'xyz.js', 'a.js'], 'count($item) > 4')",
        &context,
    )?;
...
fn test_item_context_variable() -> Result<()> {
    ...
    assert!(eval_bool(
        "any(['test.js', 'other.ts'], '$item == \"test.js\"')",
        &context
    )?);

    // Test $item in all context
    assert!(eval_bool(
        "all(['a.js', 'b.js'], 'endsWith($item, \".js\")')",
        &context
    )?);

    // Test $item in filter context
    let filtered = eval_list(
        "filter(['short', 'verylongname'], 'count($item) > 5')",
        &context,
    )?;
...
fn test_performance_with_large_collections() -> Result<()> {
    ...
    let result = call_function(
        "any",
        &[
            large_list_value.clone(),
            Value::String("$item == 'file0.js'".to_string()),
        ],
        &context,
    )?;
