...
fn create_test_context<'a>(
  ...
  if let Some(ext) = path.extension() {
    variables.insert("EXT".to_string(), Value::String(ext.to_string_lossy().to_string()));
  } else {
    variables.insert("EXT".to_string(), Value::String("".to_string()));
  }

  if let Some(stem) = path.file_stem() {
    variables.insert("BASENAME".to_string(), Value::String(stem.to_string_lossy().to_string()));
  } else {
...
fn test_variable_name() -> Result<()> {
  // Test $NAME variable
  assert_eq!(eval_expr("$NAME", "/path/to/file.js")?, Value::String("file.js".to_string()));
  assert_eq!(eval_expr("$NAME", "/simple.txt")?, Value::String("simple.txt".to_string()));
  assert_eq!(
...
fn test_variable_basename() -> Result<()> {
  ...
  assert_eq!(eval_expr("$BASENAME", "/path/to/file.js")?, Value::String("file".to_string()));
  assert_eq!(eval_expr("$BASENAME", "/simple.txt")?, Value::String("simple".to_string()));
  assert_eq!(
    eval_expr("$BASENAME", "/component.test.js")?,
    Value::String("component.test".to_string())
  );
  assert_eq!(eval_expr("$BASENAME", "/noextension")?, Value::String("noextension".to_string()));
...
fn test_variable_ext() -> Result<()> {
  // Test $EXT variable
  assert_eq!(eval_expr("$EXT", "/path/to/file.js")?, Value::String("js".to_string()));
  assert_eq!(eval_expr("$EXT", "/simple.txt")?, Value::String("txt".to_string()));
  assert_eq!(eval_expr("$EXT", "/component.test.tsx")?, Value::String("tsx".to_string()));
  assert_eq!(eval_expr("$EXT", "/noextension")?, Value::String("".to_string()));
...
fn test_string_literals() -> Result<()> {
  ...
  assert_eq!(eval_expr("\"hello world\"", "/test.js")?, Value::String("hello world".to_string()));
  assert_eq!(eval_expr("'single quotes'", "/test.js")?, Value::String("single quotes".to_string()));
  assert_eq!(eval_expr("\"\"", "/test.js")?, Value::String("".to_string()));
...
fn test_matches_function() -> Result<()> {
  ...
  assert!(is_true("matches($BASENAME, /^[a-z0-9]+(?:-[a-z0-9]+)*$/)", "/path/hello-world.js"));
  assert!(is_false("matches($BASENAME, /^[a-z0-9]+(?:-[a-z0-9]+)*$/)", "/path/HelloWorld.js"));

  // Test PascalCase pattern
  assert!(is_true("matches($BASENAME, /^[A-Z][a-zA-Z0-9]*$/)", "/path/HelloWorld.js"));
  assert!(is_false("matches($BASENAME, /^[A-Z][a-zA-Z0-9]*$/)", "/path/hello-world.js"));
...
fn test_complex_logical_expressions() -> Result<()> {
  ...
  assert!(
    is_true("($EXT == \"js\" || $EXT == \"ts\") && matches($BASENAME, /^[a-z]/)", "/component.js")
  );
  assert!(
    is_false("($EXT == \"js\" || $EXT == \"ts\") && matches($BASENAME, /^[A-Z]/)", "/component.js")
  );

  // Test complex NOT expressions
  assert!(is_true("!($EXT == \"py\" || $EXT == \"rb\")", "/component.js"));
  assert!(is_false("!($EXT == \"js\" || $EXT == \"ts\")", "/component.js"));
...
fn test_custom_matcher_combinations() -> Result<()> {
  ...
  custom_matchers.insert("PascalCase".to_string(), parse_expression(PASCAL_CASE_EXPR)?);
...
fn test_real_world_scenarios() -> Result<()> {
  ...
  assert!(is_true("matches($BASENAME, /^[A-Z][a-zA-Z0-9]*$/) && $EXT == \"jsx\"", "/Button.jsx"));
  assert!(is_false("matches($BASENAME, /^[A-Z][a-zA-Z0-9]*$/) && $EXT == \"jsx\"", "/button.jsx"));
