...
fn test_evaluate_literals() -> Result<()> {
  ...
  let expr = Expression::ListLiteral(
    vec![Expression::StringLiteral("a".to_string()), Expression::StringLiteral("b".to_string())]
  );
  let result = evaluate(&expr, &context)?;
...
fn test_evaluate_references() -> Result<()> {
  ...
  custom_matchers.insert("js-file".to_string(), Expression::BinaryOp {
    op: BinaryOperator::Equal,
  ...
  custom_matchers.insert("test-js-file".to_string(), Expression::BinaryOp {
    op: BinaryOperator::And,
...
fn test_evaluate_complex_expressions() -> Result<()> {
  ...
  let expr = parse_expression(
    "(js-file || $EXT == \"ts\") && (test-file || matches($NAME, /demo/))"
  )?;
  let result = evaluate(&expr, &context)?;
  assert!(matches!(result, Value::Boolean(true)));
